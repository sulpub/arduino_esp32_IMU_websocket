...
#define NB_PICTURES   11               // ---> MODIFY depending number of picture for the slideshow
...
const char* ota_password    = "MODIFY_OTA_PASSWORD";

//device type name for OTA
#if (DEVICE==1)
const char* device_name_ota = "ESP32_m5stack";
#endif

#if (DEVICE==2)
const char* device_name_ota = "ESP32_m5stick";
#endif

//websocket server using port 81
WebSocketsServer webSocket = WebSocketsServer(81);

//NEOPIXEL
Adafruit_NeoPixel pixels = Adafruit_NeoPixel(M5STACK_FIRE_NEO_NUM_LEDS, M5STACK_FIRE_NEO_DATA_PIN, NEO_GRB + NEO_KHZ800);
bool neopixel_power_on = true;
...
int int_num_socket = -1;
int int_old_num_socket = -1;
...
/*
   _______                      .__              .__
   \      \   ____  ____ ______ |__|__  ___ ____ |  |
   /   |   \_/ __ \/  _ \\____ \|  \  \/  // __ \|  |
  /    |    \  ___(  <_> )  |_> >  |>    <\  ___/|  |__
  \____|__  /\___  >____/|   __/|__/__/\_ \\___  >____/
          \/     \/      |__|            \/    \/
*/
//neopixel function
void neopixel_color(int r, int v, int b, int wait_time) {
  if (neopixel_power_on == true)
  {
    if (DEVICE == 1)   //1_M5STACK_FIRE 2_M5STICK
    {
      for (int_i = 0; int_i < NEOPIXEL_NUMBER; int_i++)
      {
        pixels.setPixelColor(int_i, pixels.Color(r, v , b));
        pixels.show();
        delay(5);
      }
      //pixels.show();
...
void hexdump(const uint8_t *mem, uint32_t len) {
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
  ...
  switch (type) {
    case WStype_DISCONNECTED:
      {
        ...
        int_num_socket = -1;
        ulong_time_old_screen = millis() + TIME_SCREEN_ON;
        bool_active_screen = true;
        bool_onetime_screen = true;
      }
      break;
    case WStype_CONNECTED:
      {
        int_num_socket = num;
        ip_socket = webSocket.remoteIP(num);
        Serial.printf("[%u] Connected from %d.%d.%d.%d url: %s\n", num, ip_socket[0], ip_socket[1], ip_socket[2], ip_socket[3], payload);
    ...
    case WStype_TEXT:
      ...
      //test neoxpixel interract
      buffer_ws[0] = payload[0];
      buffer_ws[1] = payload[1];
      buffer_ws[2] = payload[2];

      //color_ws= atoi(buffer_ws);

      // send message to client
      sprintf(buffer_ws, "%d", int_acc_x);
      webSocket.sendTXT(num, buffer_ws);
...
{
  flip_btnB_bool = !flip_btnB_bool;
  neopixel_power_on = !neopixel_power_on;
  ...
  if (flip_btnB_bool)
  {
    ...
    //information
    Serial.println("FLIP - Power on backlight LCD\n");
    neopixel_power_on = true;
  }
  else
  {
    pinMode(LCDBACKLIGHT, OUTPUT);
    ...
    //information
    Serial.println("FLIP - Power off backlight LCD\n");
    neopixel_power_on = false;
  }
}
...
{
  flip_btnC_bool = !flip_btnC_bool;

  if (flip_btnC_bool)
  {
    //information
    Serial.println("FLIP - 1 BtnC\n");
  }
  else
  {
    //information
    Serial.println("FLIP - 0 BtnC\n");
  }
}
...
/*
  ...
  /_______  /\___  >__| |____/|   __/    \__/\  / |___|\___  /   |___|
          \/     \/           |__|            \/           \/
*/
int indice_network = 0;
void setup_wifi() {

  bool status_wifi = false;

  //neopixel red
  neopixel_color(20, 0, 0, 0);
  neopixel_color(20, 0, 0, 100);
  ...
  for (int i = 0; i < NUM_NETWORKS; i++) {
    ...
    if (SCREEN_M5STICK && (DEVICE == 2))
    {
      u8g2.firstPage();
      do {
        u8g2.setColorIndex(1);
        u8g2.drawUTF8(10, 20, "Search");  //64x128
        u8g2.drawUTF8(10, 34, "WiFi");
        u8g2.drawUTF8(10, 48, ssidTab[i]);
      } while ( u8g2.nextPage() );
    }
    ...
    if (SCREEN_M5STACK && (DEVICE == 1))
    {
      ...
      M5.Lcd.setCursor(27, 156); M5.Lcd.print("Search");
      M5.Lcd.setCursor(27, 172); M5.Lcd.print("Wifi ");
      M5.Lcd.setCursor(27, 188); M5.Lcd.print(ssidTab[i]);
      M5.Lcd.setCursor(27, 204);  M5.Lcd.print(".");
    }

    //neopixel blue
    neopixel_color(0, 0, 20, 0);
    neopixel_color(0, 0, 20, 100);
    ...
    for (int j = 0; j < 10; j++)
    {
      if (WiFi.status() != WL_CONNECTED)
      {
        ...
        if (SCREEN_M5STICK && (DEVICE == 2))
        {
          //u8g2.firstPage();  //erase screen
          do {
            u8g2.drawUTF8(10+(j*4), 62, ".");
          } while ( u8g2.nextPage() );
        }
      }
      else
  ...
  {
    //neopixel green
    neopixel_color(0, 20, 0, 10);
    neopixel_color(0, 20, 0, 100);
    ...
    if (SCREEN_M5STACK && (DEVICE == 1))
    {
      ...
      M5.Lcd.setCursor(27, 156); M5.Lcd.print("WiFi connected");
      M5.Lcd.setCursor(27, 172); M5.Lcd.print("IP Address: ");
      M5.Lcd.setCursor(27, 188); M5.Lcd.print(WiFi.localIP());
      M5.Lcd.setCursor(27, 204); M5.Lcd.print("SSID-"); M5.Lcd.print(ssidTab[indice_network]);
      delay(2000);
    }
    ...
    if (SCREEN_M5STICK  && (DEVICE == 2))
    {
      ...
      do {
        u8g2.setColorIndex(1);
        u8g2.drawUTF8(10, 20, "WiFi");  //64x128
        u8g2.drawUTF8(10, 34, "OK");
        sprintf(buffer_screen, "%d.%d.%d.%d", Ip_Node_adress[0], \
                Ip_Node_adress[1], \
                Ip_Node_adress[2], \
                Ip_Node_adress[3]); \
        //also pseudocode.
        u8g2.drawUTF8(10, 48, buffer_screen);
        u8g2.drawXBM( 80, 10, 16, 16, logo16x16_smiley);
      } while ( u8g2.nextPage() );
  ...
  if (WiFi.status() != WL_CONNECTED)
  {
    ...
    if (nb_test_ctl_wifi > TEST_WIFI)
    {
      ...
      if (SCREEN_M5STACK && (DEVICE == 1))
      {
        ...
        M5.Lcd.setCursor(27, 156); M5.Lcd.print("WiFi disconnected");
        M5.Lcd.setCursor(27, 172); M5.Lcd.print("Network Problem");
      }
      ...
      if (SCREEN_M5STICK  && (DEVICE == 2))
      {
        ...
        do {
          u8g2.setColorIndex(1);
          u8g2.drawUTF8(10, 20, "WiFi");  //64x128
          u8g2.drawUTF8(10, 34, "KO (Problem)");
          u8g2.drawXBM( 80, 10, 16, 16, logo16x16_smiley_bored);
        } while ( u8g2.nextPage() );
...
/*
  ...
  \____|__  /___|  /__|__|_|  (____  /__| |__|\____/|___|  /
          \/     \/         \/     \/                    \/
*/
void picture_animation(bool animate, int picture)
{

  if (animate == true)
  {
    rot_img++;
    if (rot_img > (NB_PICTURES-2)) rot_img = 0;
  } // end if animate
  else
  {
    rot_img = picture;
    if (rot_img > NB_PICTURES) rot_img = 0;
  }

  Serial.print("rot_img - ");
  Serial.println(rot_img);

  switch (rot_img) {
    case 0:
      // statements
      Serial.println("Load sulpub.jpg on screen");
      M5.Lcd.drawJpgFile(SPIFFS, "/sulpub.jpg", 0, 0);
      //neopixel bleu
      neopixel_color(0, 0, 20, 0);
      neopixel_color(0, 0, 20, 100);
      break;
    case 1:
      ...
      M5.Lcd.drawJpgFile(SPIFFS, "/sulpub.jpg", 0, 0);
      //neopixel blanc
      neopixel_color(10, 10, 10, 0);
      neopixel_color(10, 10, 10, 100);
      break;
    case 2:
      // statements
      Serial.println("Load sulpub.jpg on screen");
    ...
    case 9:
      ...
      break;
    case 10:
      // cadre infos textuelle
      Serial.println("Load informations.jpg on screen");
      M5.Lcd.drawJpgFile(SPIFFS, "/informations.jpg", 0, 0);
      break;
    case 11:
      // help
      Serial.println("Load notice.jpg (help) on screen");
      M5.Lcd.drawJpgFile(SPIFFS, "/notice.jpg", 0, 0);
      break;
    default:
      // statements
      Serial.println("Load default sulpub.jpg on screen");
      M5.Lcd.drawJpgFile(SPIFFS, "/sulpub.jpg", 0, 0);
      break;
  }
...
void setup()
{
  ...
  //neopixel red
  neopixel_color(2, 0, 0, 10);
  neopixel_color(2, 0, 0, 100);
  ...
  if (DEVICE == 1)   //1_M5STACK_FIRE 2_M5STICK
  {
    ...
    //splash screen
    //if ((SCREEN_M5STACK == true) && DEVICE == 1) picture_animation(false, 0);
    //delay(2000);

    //splash infos
    if ((SCREEN_M5STACK == true) && DEVICE == 1) picture_animation(false, 11);
    delay(5000);

    //screen information cadre texte noir
    if ((SCREEN_M5STACK == true) && DEVICE == 1) picture_animation(false, 10);
    ...
    if (SCREEN_M5STACK && (DEVICE == 1))
    {
      ...
      M5.Lcd.setCursor(27, 172); M5.Lcd.print("    Program Started  ");
      delay(1000);
      ...
      if (bool_sdcard_error == true)
      {
        ...
        M5.Lcd.setCursor(27, 172); M5.Lcd.print("     SD card error     ");
        M5.Lcd.setCursor(27, 188); M5.Lcd.print("   or SD not present   ");
  ...
  if (DEVICE == 2)   //1_M5STACK_FIRE 2_M5STICK
  {
    //information
    Serial.println("Config BtnPin M5Stick");
    pinMode(BtnPin, INPUT_PULLUP);
    attachInterrupt(BtnPin, pressed, FALLING); /*Check if the button is pushed using interrupt*/
    delay(50);
    btnState = 0;
  } //if DEVICE == 2
  ...
  if (SCREEN_M5STICK  && (DEVICE == 2))
  {
    ...
    u8g2.noDisplay(); //turn off display
    u8g2.display(); //turn on display
    u8g2.firstPage();
    do {
      u8g2.setColorIndex(1);
      u8g2.drawUTF8(10, 20, "Start");  //64x128
      u8g2.drawUTF8(10, 34, "Prog");
    } while ( u8g2.nextPage() );
  }
  ...
  ArduinoOTA.setHostname(device_name_ota);
  // Authentication for OTA (security for limit upgrade)
  ...
  //neopixel off
  neopixel_color(0, 0, 0, 10);
  neopixel_color(0, 0, 0, 100);
  ...
  //diaporama
  //animation M5STACK
  bool exit_diapo = true;
  if (SCREEN_M5STACK && DEVICE == 1)  //1_M5STACK_FIRE 2_M5STICK
  {
    while (exit_diapo == true)
    {
      ...
      //M5.update();
      if ((ulong_time_now >= ulong_time_old_animate) && !(bool_active_screen))
      {
        ulong_time_old_animate = millis() + WAIT_DIAPORAMA;
        Serial.println("-- UPDATE PICTURE --");
        picture_animation(true, 0);  //picture_animation(bool animate, int picture)
      }

      if (flip_btnC_bool) exit_diapo = false;

      //sleep mode

      //state button for low power
      if (btnState == 1)
      {
        //neopixel off
        neopixel_color(0, 0, 0, 10);
  ...
  }//diaporama only on M5STACK

  //screen information
  if ((SCREEN_M5STACK == true) && DEVICE == 1) picture_animation(false, 10);

  //neopixel black
  neopixel_color(0, 0, 0, 0);
  neopixel_color(0, 0, 0, 100);
...
void loop()
{
  //information connexion websocket
  if (int_num_socket != int_old_num_socket)
  {
    int_old_num_socket = int_num_socket;

    //M5STICK SCREEN
    if (SCREEN_M5STICK && bool_active_screen  && (DEVICE == 2))
    {
      if (bool_onetime_screen == true)
      {
        bool_onetime_screen = false;
        u8g2.display(); //turn on display
        u8g2.firstPage();
        do {
          u8g2.setColorIndex(1);
          if (int_num_socket == -1)
          {
            u8g2.drawUTF8(10, 20, "Socket");  //64x128
            u8g2.drawUTF8(10, 34, "Disconnected");
          }
          else
          {
            u8g2.drawUTF8(10, 20, "Socket");  //64x128
            u8g2.drawUTF8(10, 34, "Connected");
            sprintf(buffer_screen, "%d.%d.%d.%d", ip_socket[0], \
                    ip_socket[1], \
                    ip_socket[2], \
                    ip_socket[3]); \
            //also pseudocode.
    ...
    } //end if M5STICK SCREEN

    //M5STACK SCREEN
    if (SCREEN_M5STACK && bool_active_screen  && (DEVICE == 1))
    {
      if (bool_onetime_screen == true)
      {
        bool_onetime_screen = false;
        ...
        if (int_num_socket == -1)
        {
          ...
          M5.Lcd.setCursor(27, 156); M5.Lcd.print("Socket");
          M5.Lcd.setCursor(27, 172); M5.Lcd.print("Disconnected");
        }
        else
        {
          M5.Lcd.setTextColor(GREEN , BLACK);
          ...
          M5.Lcd.setCursor(27, 156); M5.Lcd.print("Socket");
          M5.Lcd.setCursor(27, 172); M5.Lcd.print("Connected");
          M5.Lcd.setCursor(27, 188); M5.Lcd.print(ip_socket);
        }
      } //if screen one time
    }

  }  //end if websocket connexion screen info
  ...
  if (ulong_time_now >= ulong_time_old_screen)
  {
    ...
    if (SCREEN_M5STACK && bool_onetime_static_mode && (DEVICE == 1) && !(bool_motion_detected) )
    {
      ...
      //neopixel black
      if (flip_neopixel_detect == true)
      {
        neopixel_color(0, 0, 0, 0);
        neopixel_color(0, 0, 0, 100);
        flip_neopixel_detect = false;
      }
  ...
  if (DEVICE == 2)  //1_M5STACK_FIRE 2_M5STICK
  {
    ...
    {
      //information
      Serial.println("M5STICK Enter in Low Power\n");
      esp_deep_sleep_start();
    }
  }
  ...
  //sending quaternion
  sprintf(buffer_ws, "{\"q0\":%d,\"q1\":%d,\"q2\":%d,\"q3\":%d,\"Tu\":%d}", \
          (int)(q[0] * 1000), (int)(q[1] * 1000), (int)(q[2] * 1000), (int)(q[3] * 1000), \
          int_intTemp_meas);
  webSocket.sendTXT(int_num_socket, buffer_ws);


  //condition for activate screen
  if ( (gx > GYR_THRESHOLD) || (gx < -GYR_THRESHOLD) || \
       (gy > GYR_THRESHOLD) || (gy < -GYR_THRESHOLD) || \
       (gz > GYR_THRESHOLD) || (gz < -GYR_THRESHOLD))
  {
    ulong_time_old_screen = millis() + TIME_SCREEN_ON;
    bool_active_screen = true;
    bool_onetime_screen = true;
    bool_motion_detected = true;
    //M5STACK
    bool_onetime_static_mode = true;
  } //end if motion detected

  //M5STICK Message IN MOTION
  if (SCREEN_M5STICK && bool_active_screen  && (DEVICE == 2))
  {
    if ((bool_onetime_screen == true) && (bool_motion_detected == true))
    {
      bool_onetime_screen = false;
      bool_motion_detected = false;
      u8g2.display(); //turn on display
      u8g2.firstPage();
      do {
        u8g2.setColorIndex(1);
        u8g2.drawUTF8(10, 20, "Motion");  //64x128
        u8g2.drawUTF8(10, 34, "Detected.");
        sprintf(buffer_screen, "%d.%d.%d.%d", Ip_Node_adress[0], \
                Ip_Node_adress[1], \
                Ip_Node_adress[2], \
                Ip_Node_adress[3]); \
  ...
  if (SCREEN_M5STACK && bool_active_screen  && (DEVICE == 1))
  {
    if ((bool_onetime_screen == true) && (bool_motion_detected == true))
    {
      ...
      M5.Lcd.setCursor(27, 156); M5.Lcd.print("                       ");
      M5.Lcd.setCursor(27, 172); M5.Lcd.print("   Motion Detected     ");
      M5.Lcd.setCursor(27, 188); M5.Lcd.print("                       ");
      M5.Lcd.setCursor(27, 204); M5.Lcd.print("                       ");
      ...
      //neopixel red
      if (flip_neopixel_detect == false)
      {
        neopixel_color(20, 0, 0, 0);
        neopixel_color(20, 0, 0, 100);
        flip_neopixel_detect = true;
      }
    } //if screen one time
  }
  ...
  if (!AHRS) {
    ...
    if (delt_t > 500) { // update LCD once per half-second independent of read rate
      ...
      if (SERIAL_DEBUG || (uart_mode == 1)) {
        ...
        Serial.print(" Tcpu = "); Serial.println(int_intTemp_meas);
